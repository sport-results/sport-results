...
export class UserFormService extends ComponentStore<UserFormState> {
  ...
  private readonly selectUserEntity = this.effect(
    (userId$: Observable<string | undefined>) => {
      return userId$.pipe(
        tap((userId) => {
          const userEntity = this.userStoreService.selectEntityById(userId || '')();
  ...
  private addUser(formGroup: FormGroup): void {
    this.userStoreService.addEntity(
  ...
  private updateUser(formGroup: FormGroup): void {
    this.userStoreService.updateEntity(
      this.userUtilService.updateEntity(formGroup) as UserEntity
    );
  }
