...
const permissionReducer = createReducer(
    ...
    on(permissionActions.loadEntitySuccess, (state, { permission }) => {
        if (permission) {
            return permissionAdapter.upsertOne(permission, { ...state, loading: false });
        } else {
            return state;
        }
    }),
    ...
    on(permissionActions.listEntitiesSuccess, (state, { permissions }) =>
    ...
    on(permissionActions.updateEntityFail, (state, { error }) => ({ ...state, error }))
);
