...
export const getEntityBySportEventId = (sportEventId: string) =>
  createSelector(getAll, (sportResults: SportResultEntity[]) => {
    return (sportResults || []).filter(
      (sportResult) =>
        sportResult.path.find(
          (pathItem) => pathItem.key === SPORT_EVENT_FEATURE_KEY
        )?.value === sportEventId
    )[0];
  });
